...
- Open `index.html` and replace `github.com/your-username` in the social
  row with your actual GitHub profile URL.
- Add your own photo: drop an image file (e.g. `photo.jpg`) into the repo,
  then in `index.html` replace the placeholder SVG inside `.photo-blob` with
  `<img src="photo.jpg" alt="Vidath Kodippili">`. The blob shape and border
  are on `.photo-blob` in `styles.css`, so any photo you drop in will pick up
  the same frame automatically.
- I left your phone number off the page on purpose — a public phone number
  tends to get scraped. Add it back into the social row if you'd rather have
  it visible.
